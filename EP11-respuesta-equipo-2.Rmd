...
Como las variables elegidas fueron Elbows.diameter, Waist.Girth, Hip.Girth, Thigh.Girth, Bicep.Girth y Height son medidas, también se cumple esta condición
...
```
Para evitar problemas eliminamos el predictor Hip.Girth, ya que tienen un VIF preocupante y volvemos a generar el modelo y evaluar la multicolinealidad
```{r}
modelo_variables = modelo_variables[-3]
texto <- paste(modelo_variables, collapse = " + ")
modelo_formula <- formula(paste(variable_a_predecir, texto, sep = " ~ "))
...
print(vif(modelo_final))
```
Como se puede ver el predictor Elbows.diameter  tiene un VIF preocupante por lo que repetimos el procedimiento anterior con esta variable
```{r}
