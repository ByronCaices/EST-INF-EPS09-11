...
```
Notamos que el caso 10, 11, 131 y 141 podrían considerarse problemáticos por la distancia de Cook que presentan. Para evaluar estos utilizaremos la funcion `compareCoefs`
...
Podemos ver que nínguno de los casos provaca cambios en los coeficientes mucho mayores que los demás, por lo tanto no se debería elimina ningún caso, cabe resaltar que el caso 131 es el que provoca los mayores cambios, por lo que si bien no es significativo frente a los demás provamosel modelo eliminandolo
...
# 10, 11, 22, 181
```
Notamos que el caso 11 y 22 **podrían** considerarse problemáticos por la distancia de Cook que presentan. Para evaluar estos utilizaremos la funcion `compareCoefs` nuevamente
...
```
La comparación muestra que remover la mayoría de los casos no altera de manera significativa los coeficientes (cambios de 0% a aproximadamente 1%). Sin embargo, eliminar el caso 22 produce los mayores cambios porcentuales, llegando hasta un 1.7% en las estimaciones. Esto indica que el caso 47 es el más influyente y su remoción sería la más conveniente para mejorar el ajuste del modelo.
