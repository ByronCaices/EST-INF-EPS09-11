...
El valor p (0.8628) es muy alto, lo que indica que no hay evidencia estadísticamente significativa de curvatura en la relación entre la variable Waist.Girth y el logit de la variable respuesta. En otras palabras, no se detecta una violación del supuesto de linealidad en el logit para este predictor. Esta variable puede considerarse adecuadamente modelada de forma lineal.

Modelo multivariado (con Thigh.Girth, Wrist.Minimum.Girth y Height):
Para cada uno de los predictores (Thigh.Girth, Wrist.Minimum.Girth, Height), los valores p (0.4778, 0.1507, 0.3766 respectivamente) también indican ausencia de evidencia significativa de curvatura. Esto sugiere que ninguna de estas variables muestra un patrón curvo o no lineal importante respecto a la respuesta en el modelo. Cada predictor parece adecuadamente capturado mediante una relación lineal en el logit.

#### Casos sobre influyentes

Revisemos los residuos de student, apalancamiento y disancias de cook (representada por las áreas de los círculos)
...
```
Notamos que el caso 45, 251 y 25 podrían considerarse problemáticos por la distancia de Cook que presentan. Para evaluar estos utilizaremos la funcion `compareCoefs`
...
Cambio <- abs((modelo_comp[, 1]-modelo_comp[, 3])/modelo_comp[,1])
...
Podemos ver que los cassos 25 y 45 provocan cambios en los coeficientes considereblamente mayores respecto a los demás cassos por lo que es conveniente eliminarles del modelo

```{r Eliminar casos problemáticos}

modelo <- update(modelo, subset = -c(25,45))
...
# 47, 81, 15, 481, 39
```
Notamos que el caso 47, 81, 15, 481, 39 **podrían** considerarse problemáticos por la distancia de Cook que presentan. Para evaluar estos utilizaremos la funcion `compareCoefs` nuevamente
...
```
La comparación muestra que remover la mayoría de los casos no altera de manera significativa los coeficientes (cambios de 0% a aproximadamente 1%). Sin embargo, eliminar el caso 47 produce los mayores cambios porcentuales, llegando hasta un 4.5% en las estimaciones. Esto indica que el caso 47 es el más influyente y su remoción sería la más conveniente para mejorar el ajuste del modelo.

Los otros casos (15, 81, 481, 39) muestran cambios muy cercanos a cero en los coeficientes al ser removidos, lo que indica que su influencia sobre el modelo es mínima. Por el contrario, el caso 47 genera cambios porcentuales notables (superiores al 3%) en varios coeficientes. Esta diferencia de impacto sugiere que el caso 47 es claramente el más influyente y problemático, y por lo tanto, su eliminación es la que más podría beneficiar la mejora del modelo.

```{r Eliminar casos problemáticos M}

modelo3 <- update(modelo3, subset = -c(47))
...
1. **Modelo Univariado:**  
   - Sensibilidad: 0.84  
     Esto significa que el 84% de las personas con sobrepeso (EN = 1) fueron correctamente identificadas por el modelo.  
   - Especificidad: 0.80  
     El 80% de las personas sin sobrepeso (EN = 0) fueron clasificadas correctamente como tales.  

   En términos generales, el modelo univariado realiza un buen trabajo identificando casos positivos y negativos, pero su especificidad podría ser un poco mejor.

2. **Modelo Multivariado:**  
   - Sensibilidad: 0.84  
     Al igual que el univariado, el modelo multivariado identifica correctamente al 84% de las personas con sobrepeso.  
   - Especificidad: 0.84  
     El modelo multivariado mejora en la identificación correcta de personas sin sobrepeso, llegando a un 84% de especificidad.

En comparación con el modelo univariado, el modelo multivariado mantiene la misma capacidad para detectar casos positivos (misma sensibilidad), pero mejora en la identificación de los casos negativos (aumenta la especificidad). Esto lo convierte en un modelo más balanceado a la hora de predecir el estado nutricional, mostrando un mejor desempeño global en el conjunto de prueba.
