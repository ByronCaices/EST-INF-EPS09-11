...
Primero dejamos los datos solo con los posibles predictores y el escogido en el paso anterior
```{r datos ajustados}
datosEntrenamiento = datosEntrenamiento %>% select(c("Knee.Girth", "Weight", "Chest.diameter", "Wrist.Minimum.Girth", "Thigh.Girth", "Height", "Calf.Maximum.Girth", "Gender", "Waist.Girth", "EN" ))
datosPrueba = datosPrueba %>% select(c("Knee.Girth", "Weight", "Chest.diameter", "Wrist.Minimum.Girth", "Thigh.Girth", "Height", "Calf.Maximum.Girth", "Gender", "Waist.Girth", "EN" ))
```
Para esto usaremos regresion paso a paso escalonada
```{r modelo multiple}
modeloNulo <- glm(EN ~ Waist.Girth, data = datosEntrenamiento, family = binomial(link = "logit"))
...
```
Luego de aplicar la regresión paso a paso escalonada obtenemos un modelo en base a la altura y el peso adicional al grosor de la cintura , lo cual tiene todo el sentido, puesto que el estado nutricional depende del IMC, el cual es calculado en base a la altura y el peso de la persona
